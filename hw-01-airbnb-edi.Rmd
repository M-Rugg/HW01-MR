...
There are `r nrow(edibnb)` rows in the dataset.
...
edibnb_top_5 <- edibnb %>%
  filter(neighbourhood == "New Town" | neighbourhood == "West End" | neighbourhood == "Old Town" | neighbourhood == "Stockbridge" | neighbourhood == "Bruntsfield" )

library(ggridges)

ggplot(data = edibnb_top_5, mapping = aes(x = price, y = neighbourhood, color = neighbourhood, fill = neighbourhood)) +
...
```{r}
ggplot(data = edibnb, mapping = aes(x = review_scores_rating, y = neighbourhood, color = neighbourhood, fill = neighbourhood)) +
